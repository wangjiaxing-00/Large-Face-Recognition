...
def download(sub_dir, name, force=False, root='~/.rflm106det'):
...
def ensure_available(sub_dir, name, root='~/.rflm106det'):
    return download(sub_dir, name, force=False, root=root)

def download_onnx(sub_dir, model_file, force=False, root='~/.rflm106det', download_zip=False):
    _root = os.path.expanduser(root)
