...
from face_detect.rfdet.layers import FPN, SSH
from face_detect.rfdet.mb import MBNet
...
class FaceDetectRFNet(nn.Module):
    def __init__(self, cfg = None, pretrained = False, mode = 'train'):
        super(FaceDetectRFNet,self).__init__()
        backbone = None
        if cfg['name'] == 'mb':
            backbone = MBNet()
            if pretrained:
                checkpoint = torch.load("./facemodel/mb_pre.tar", map_location=torch.device('cpu'))
